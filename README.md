...
| `mosquitto.conf` | mosquitto 브로커 설정 (외부 접속 허용, 1순위 권장) |
...
- **Windows**: https://mosquitto.org/download/ 에서 설치 → 설치 폴더의 `mosquitto.exe`를 이 프로젝트의 `mosquitto.conf`로 실행. Windows 방화벽 허용 창이 뜨면 **개인/공용 네트워크 모두 허용** (안 하면 ESP32 접속 불가).
- **macOS**: `brew install mosquitto` / **Linux**: `apt install mosquitto`
- **핵심 주의**: mosquitto 2.x는 기본이 localhost 전용. 반드시 `-c mosquitto.conf`로 실행해야 ESP32가 접속 가능 (`listener 1883` + `allow_anonymous true`).
- `-v` 모드에서는 수신 토픽이 콘솔에 찍히므로, ESP32가 발행을 제대로 하는지 서버와 무관하게 즉시 확인 가능 — 당일 디버깅 1차 도구로 사용.
- 접속 테스트: `mosquitto_sub -h <노트북IP> -t "safeon/#" -v` (동봉 CLI 도구)

## ESP32 전환

1. 관제 노트북에서 `mosquitto -c mosquitto.conf -v` + 서버 실행, 노트북 IP 확인 (예: 192.168.0.10)
